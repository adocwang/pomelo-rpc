        ...
        socket.on('message', function (data, flags) {
            try {
                ...
                if (msg instanceof Array) {
                    processMsgs(socket, self, msg);
                } else {
                    processMsg(socket, self, msg);
                }
            } catch (e) {
...
var processMsg = function (socket, acceptor, pkg) {
    ...
    acceptor.cb(tracer, pkg.msg, function () {
        ...
        if (acceptor.bufferMsg) {
            ...
            if (this.rpcDebugLog) {
                doSend(socket, JSON.stringify(resp));
            } else {
                doSend(socket, StrMsgEncoder.makeDataPieceStr(resp));
            }
        }
    });
};
...
var flush = function (acceptor) {
    ...
    for (var socketId in queues) {
        ...
        let pieces = [];
        for (let dataObj of queue) {
            if (this.rpcDebugLog) {
                pieces.push(JSON.stringify(dataObj));
            } else {
                pieces.push(StrMsgEncoder.makeDataPieceStr(dataObj))
            }
        }
        doSend(socket, StrMsgEncoder.makeDataArray(pieces));
